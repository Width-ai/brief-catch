...
async def bulk_ngram_analysis(csv_file: UploadFile = File(...)) -> JSONResponse:
    ...
    if (
        "//Rule" not in df.columns
        or "//Correction" not in df.columns
        or "//Number" not in df.columns
    ):
        return JSONResponse(
            status_code=500,
            content=f"Columns missing '//Correction', '//Rule' or '//Number current dataset has columns: {df.columns}",
        )
