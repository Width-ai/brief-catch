...
if modification_action == "Create":
    ...
    if st.session_state.get('form_submitted', False):
        st.session_state['new_rule'] = st.text_area(
            ...
            args=('final_rule_version',)  # Pass the key of the text area to the callback
        )
        ...
        if st.button("Update Repo"):
            new_rule_name_match = re.search(name_regex_pattern, st.session_state['new_rule'])
            if new_rule_name_match:
                new_rule_name = new_rule_name_match.group(1)
            else:
                new_rule_name = f"BRIEFCATCH_{category.upper()}_{rule_number}"
            create_pr(rule_name=new_rule_name, modified_rule=st.session_state['new_rule'])
    ...
    if st.button("Modify") and not st.session_state.get('modified', False):
        ...
        for modification in st.session_state.modifications:
            with st.spinner(f"Processing Modification {st.session_state.modifications.index(modification)+1}..."):
                ...
                if response.status_code == 200:
                    json_response = response.json()
                    st.session_state['new_rule'] = json_response["response"]  # Update the rule text for the next modification
                    # Update total usage
                    st.session_state["total_usage"]["input_tokens"] += json_response['usage']['input_tokens']
                    st.session_state["total_usage"]["output_tokens"] += json_response['usage']['output_tokens']
                    st.session_state["total_usage"]["cost"] += json_response['usage']['cost']
                else:
    ...
    if st.session_state.get('modified', False):
        ...
        st.markdown(f"Input token count: {st.session_state['total_usage']['input_tokens']}")
        st.markdown(f"Output token count: {st.session_state['total_usage']['output_tokens']}")
        st.markdown(f"Cost: {st.session_state['total_usage']['cost']}")
        if st.button("Update Repo"):
            with st.spinner("Opening Pull Request..."):
                create_pr(rule_name=rule_to_modify, modified_rule=st.session_state['new_rule'])